...
handle_event(event(EventType, DictIn)) :-
    ...
    findall(
        ...
        (
                ...
                (
                    ...
                    log_trace(info, '[Abstract] handle_event  Rule matched: ~w Priority: ~w Keys: ~w', [RuleID, Priority, Keys])
                ),
                E,
    ...
    ( RuleList \= [] ->
        log_trace(info,'[Abstract] handle_eventMatched rules: ~q', [RuleList]),
...
apply_matching_rules([abstract_rule(RuleID, Priority, Patterns, Conditions, Abstracts, Keys,Transforms)|Rest], event(EventType, DictIn)) :-
    log_trace(info, '[Abstract] apply_matching_rules Applying abstract_rule ~w (Priority: ~w)', [RuleID, Priority]),
    Patterns = [Pattern | _],
    Abstracts = [Abstract | _],
    log_trace(info, '[Abstract] apply_matching_rules Pattern: ~q, Abstract: ~q', [Pattern, Abstract]),
    Abstract =.. [AbsType, _],
...
existing_abstract(event(EventType, DictIn), Conditions, event(AbsType, AbsDict), Keys) :-
    ...
    ( find_event(AbsType, AbsDictKeys, Candidate) ->
        AbsDict = Candidate,
