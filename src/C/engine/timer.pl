...
init_queue :-
    ...
    ; message_queue_create(timer_queue),
      format('[Timer] Created message queue timer~n')
    ).

thread_goal_timer(ClientID) :-
    format('[Timer ~w] Thread started~n', [ClientID]).
...
load_timer_rules :-
    ...
    format('[Timer] Rules loaded from ~w~n', [RuleFile]).

load_timer_rules :-
    format('[Timer] Warning: Rules file not found.~n', []).
...
timer_loop :-
    ...
    (   catch(handle_event(EventTerm), E,
              (format('[Timer] Error: ~w~n', [E]), fail))
    ->  true
    ;   format('[Timer] Warning: EventTerm not handled: ~w~n', [EventTerm])
    ),
    format('[Timer] Received: ~q~n', [EventTerm]),
    timer_loop.
...
handle_event(event(EventType, DictIn)) :-
    format('[Timer] Normalized DictIn: ~q~n', [DictIn]),
    ...
    format('[Timer] Matched rules: ~q~n', [RuleList]),
    % Sort by decreasing priority PRIORITY 100 > PRIORITY 10 
    sort(1, @>=, RuleList, SortedRules),
    format('[Timer] Sorted rules by priority: ~q~n', [SortedRules]),
    apply_matching_rules(SortedRules, DictIn, DictOut),
    format('[Timer] After apply_matching_rules: ~q~n', [DictOut]),
    EventOut = event(EventType, DictOut),
    assert_event(EventOut),
    format('[Timer] Final event to assert: ~q~n', [EventOut]),
    log_event(EventOut),
