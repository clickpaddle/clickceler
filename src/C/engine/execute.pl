...
init_queue :-
    ...
    ; message_queue_create(execute_queue),
      log_trace(info,'[Execute] Created message queue execute~n',[])
    ).

thread_goal_execute(ClientID) :-
    log_trace(info,'[Execute ~w] Thread started~n', [ClientID]).
...
load_execute_rules :-
    ...
    log_trace(info,'[Execute] Rules loaded from ~w~n', [RuleFile]).

load_execute_rules :-
    log_trace(info,'[Execute] Warning: Rules file not found.~n', []).
...
execute_loop :-
    ...
    (   catch(handle_event(EventTerm), E,
              (log_trace(info,'[Execute] Error: ~w~n', [E],[]), fail))
    ->  true
    ;   log_trace(info,'[Execute] Warning: EventTerm not handled: ~w~n', [EventTerm])
    ),
    log_trace(info,'[Execute] Received: ~q~n', [EventTerm]),
    execute_loop.
...
handle_event(event(EventType, DictIn)) :-
    log_trace(info,'[Execute] Normalized DictIn: ~q~n', [DictIn]),
    ...
    log_trace(info,'[Execute] Matched rules: ~q~n', [RuleList]),
    % Sort by decreasing priority PRIORITY 100 > PRIORITY 10 
    sort(1, @>=, RuleList, SortedRules),
    log_trace(info,'[Execute] Sorted rules by priority: ~q~n', [SortedRules]),
    apply_matching_rules(SortedRules, DictIn, DictOut),
    log_trace(info,'[Execute] After apply_matching_rules: ~q~n', [DictOut]),
    EventOut = event(EventType, DictOut),
    assert_event(EventOut),
    log_trace(info,'[Execute] Final event to assert: ~q~n', [EventOut]),
    log_event(EventOut),
