...
init_queue :-
    ...
    ; message_queue_create(propagate_queue),
      log_trace(info,'[Propagate] Created message queue propagate~n',[])
    ).

thread_goal_propagate(ClientID) :-
    log_trace(info,'[Propagate ~w] Thread started~n', [ClientID]).
...
load_propagate_rules :-
    ...
    log_trace(info,'[Propagate] Rules loaded from ~w~n', [RuleFile]).

load_propagate_rules :-
    log_trace(warning,'[Propagate] Rules file not found.~n', []).
...
propagate_loop :-
    ...
    (   catch(handle_event(EventTerm), E,
              (log_trace(info,'[Propagate] Error: ~w~n', [E]), fail))
    ->  true
    ;   log_trace(info,'[Propagate] Warning: EventTerm not handled: ~w~n', [EventTerm])
    ),
    log_trace(info,'[Propagate] Received: ~q~n', [EventTerm]),
    propagate_loop.
...
handle_event(event(EventType, DictIn)) :-
    log_trace(info,'[Propagate] Normalized DictIn: ~q~n', [DictIn]),
    ...
    log_trace(info,'[Propagate] Matched rules: ~q~n', [RuleList]),
    % Sort by decreasing priority PRIORITY 100 > PRIORITY 10 
    sort(1, @>=, RuleList, SortedRules),
    log_trace(info,'[Propagate] Sorted rules by priority: ~q~n', [SortedRules]),
    apply_matching_rules(SortedRules, DictIn, DictOut),
    log_trace(info,'[Propagate] After apply_matching_rules: ~q~n', [DictOut]),
    EventOut = event(EventType, DictOut),
    assert_event(EventOut),
    log_trace(info,'[Propagate] Final event to assert: ~q~n', [EventOut]),
    log_event(EventOut).
