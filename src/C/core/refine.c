...
void *refine_handler_thread(void *arg) {
    ...
    fprintf(stderr, "[DEBUG] [Refine Thread %ld] Entering refine_handler_thread\n", logical_id);

    // Attache le thread C au moteur Prolog
    ...
    if (!PL_next_solution(q)) {
        fprintf(stderr, "[ERROR] [Refine %ld] Failed to run %s/0 (no solution or exception).\n", logical_id, predicate);
    } else {
    ...
    PL_unregister_atom(modname_atom);
    PL_thread_destroy_engine();
    fprintf(stderr, "[DEBUG] [Refine %ld] Prolog engine detached and thread exited.\n", logical_id);
    return NULL;
}
