...
int load_rules_for_thread(PL_engine_t prolog_engine, module_t out_module, atom_t modname_atom) {
    ...
    // Build file path: ../rules/<modulename>.pl
    char filename[256];
    snprintf(filename, sizeof(filename), "../rules/%s.pl", modname_str);
    ...
    if (!pred) {
        fprintf(stderr, "[ERROR] [Thread %lu] Predicate %s/1 not found in module %s\n",
                current_thread_id, goal_name, modname_str);
        return 1;
    }

    // Build Prolog call: with_output_to(string(S), thread_goal_<modname>(ThreadID))
    ...
    if (!PL_cons_functor(inner_call, PL_new_functor(PL_new_atom(goal_name), 1), thread_id_term)) {
    ...
    if (!PL_get_chars(string_var, &captured_output, CVT_ALL | CVT_WRITE | BUF_DISCARDABLE)) {
