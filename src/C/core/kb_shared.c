...
void *kb_shared_handler_thread(void *arg) {
    ...
    fprintf(stderr, "[DEBUG] [Kb_shared Thread %ld] Entering kb_shared_handler_thread\n", logical_id);
    ...
    if (pred_start) {
        ...
        if (!PL_next_solution(q)) {
            fprintf(stderr, "[ERROR] [Kb_shared %ld] Failed to run %s: no solution or error.\n", logical_id, predicate);
        } else {
            fprintf(stderr, "[DEBUG] [Kb_shared %ld] %s completed successfully.\n", logical_id, predicate);
        }
        PL_close_query(q);
    } else {
        fprintf(stderr, "[ERROR] [Kb_shared %ld] Predicate %s/0 not found in module '%s'.\n", logical_id, predicate, modname);
    }
    ...
    fprintf(stderr, "[DEBUG] [Kb_shared %ld] Prolog engine detached and thread exited.\n", logical_id);
    return NULL;
}
