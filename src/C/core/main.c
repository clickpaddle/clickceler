...
// kb_shared_thread
   ...
   if (pthread_create(&kb_shared_handler_tid, NULL, kb_shared_handler_thread, (void *)THREAD_ID_KB_SHARED) != 0) {
...
// Collector Thread
 ...
 // Refine Thread
    ...
    if (pthread_create(&refine_handler_tid, NULL, refine_handler_thread, (void *)THREAD_ID_REFINE) != 0) {
        perror("[ERR] [Main Thread] pthread_create refine_handler_thread");
        PL_halt(1);
    }
    fprintf(stderr, "[DEBUG] [Main Thread] Refine handler thread created.\n");
