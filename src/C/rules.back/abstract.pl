abstract_rule(infrastructure_issue, 100,
    ...
    [ incident(E) ],
    % Keys for abstract identification
%    [hostname, origin, subnet, source],
    % Transformations
    [ set_field(E, status, open),
      ...
      set_field(E, hostname, F.hostname),
      set_field(E, origin, F.origin),
      set_field(E, subnet, F.subnet),
      set_field(E, source, F.source)
    ]
).
